...
const arr = [
  { Icon: <MdEmail />, content: "office@zimart.com" },

  { Icon: <MdPhone />, content: "(123) 456-789" },
];
const FooterLinks = () => {
  return (
    <InViewAnimation once className="icons-footer">
      <h3 className=" footer-head ">contact me</h3>

      <div className={clsx("social-icons center")}>
        {iconsArr.map(({ icon, id, title, link }) => {
          return (
            <Title key={id} title={title}>
              <motion.a
                variants={opacityVariant}
                className={clsx("icon-parent")}
                style={{ fontSize: 30 }}
      ...
      <div>
        {arr.map(({ Icon, content }, index) => (
          <p key={index} className=" center gap">
            {Icon}
            <span>{content}</span>
          </p>
        ))}
